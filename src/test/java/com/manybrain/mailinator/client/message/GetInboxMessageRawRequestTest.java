...
class GetInboxMessageRawRequestTest
{
    ...
    void testGetInboxMessageRawRequest()
    {
        ...
        String raw = getMailinatorClient().request(new GetInboxMessageRawRequest(domain, inbox, postedMessage.getId()));
        assertNotNull(raw);
    ...
    void testGetInboxMessageRawRequestWhenMessageDoesNotExist()
    {
        String domain = getPrivateDomain();
        assertThrows(InternalServerErrorException.class, () -> getMailinatorClient().request(
                new GetInboxMessageRawRequest(domain, getInboxTest(), UUID.randomUUID().toString())));
    }
}
