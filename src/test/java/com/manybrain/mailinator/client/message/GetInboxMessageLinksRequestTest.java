...
class GetInboxMessageLinksRequestTest
{
    ...
    void testGetInboxMessageLinksRequest()
    {
        ...
        Links links = getMailinatorClient().request(new GetInboxMessageLinksRequest(domain, inbox, postedMessage.getId()));
        assertNotNull(links);
    ...
    void testGetInboxMessageLinksRequestWhenMessageDoesNotExist()
    {
        String domain = getPrivateDomain();
        assertThrows(InternalServerErrorException.class, () -> getMailinatorClient().request(
                new GetInboxMessageLinksRequest(domain, getInboxTest(), UUID.randomUUID().toString())));
    }
}
